...
contract PyStreams is Ownable, IPayStreams {
    ...
    function createStream(StreamData calldata _streamData, HookConfig calldata _streamerHookConfig, string memory _tag)
        external
    {
        if (
    ...
    function getStreamHash(address _streamer, address _recipient, address _token, string memory _tag)
        public
        pure
        returns (bytes32)
    {
